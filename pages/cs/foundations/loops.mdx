...
called a __loop__. In the previous section, we the most basic data
...
1. `int i = 0;` is the __counter__, a variable to be used by the test
...
called the for-loop's __header__, and the block of code to execute is
...
an __indefinite loop__. If, however, we know exactly how many times we want
to iterate or we have data that provides such number, a for-loop is ideal
&mdash; it is a __definite loop__.
