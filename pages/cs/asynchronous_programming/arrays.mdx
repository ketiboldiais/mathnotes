...
The values in an array are __indexed__, meaning that each value in the
array has an __index__, or position within the array, starting from 0. So,
for example:
...
The `includes()` method is a Boolean method—it returns either true or
...
the array, JavaScript stores the array's __reference__. An array's
