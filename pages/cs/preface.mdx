# Review of Computer Science


<div className="illus BinaryTree">
	<div
		className="svgContainer"
		style={{
			display: "block",
			position: "relative",
			width: "80%",
			paddingBottom: "40%",
			backgroundColor: "inherit",
			overflow: "hidden",
		}}>
		<svg
			className="illus"
			preserveAspectRatio="xMinYMin meet"
			viewBox="0 0 300 150"
			style={{
				display: "inline-block",
				position: "absolute",
				top: 0,
				left: 0,
				backgroundColor: "inherit",
			}}
			{...props}>
			<g className="svgElement" transform="translate(20, 20)">
				<g className="binaryTree">
					<g className="binaryTreeEdgeGroup">
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={171.722972972973}
							y1={0}
							x2={120.33783783783784}
							y2={25}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={171.722972972973}
							y1={0}
							x2={223.10810810810813}
							y2={25}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={120.33783783783784}
							y1={25}
							x2={73.78378378378378}
							y2={50}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={120.33783783783784}
							y1={25}
							x2={166.8918918918919}
							y2={50}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={223.10810810810813}
							y1={25}
							x2={205.54054054054058}
							y2={50}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={223.10810810810813}
							y1={25}
							x2={240.6756756756757}
							y2={50}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={73.78378378378378}
							y1={50}
							x2={36.89189189189189}
							y2={75}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={73.78378378378378}
							y1={50}
							x2={110.67567567567568}
							y2={75}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={166.8918918918919}
							y1={50}
							x2={149.32432432432432}
							y2={75}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="none"
							x1={166.8918918918919}
							y1={50}
							x2={184.45945945945948}
							y2={75}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={36.89189189189189}
							y1={75}
							x2={19.32432432432432}
							y2={100}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={36.89189189189189}
							y1={75}
							x2={54.45945945945945}
							y2={100}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="initial"
							x1={110.67567567567568}
							y1={75}
							x2={93.10810810810811}
							y2={100}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
						<line
							className="binaryTreeEdge"
							display="none"
							x1={110.67567567567568}
							y1={75}
							x2={128.24324324324326}
							y2={100}
							stroke="black"
							strokeOpacity={1}
							markerEnd="url(#arrow_end)"
							strokeWidth={1}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeBranchNode"
							cx={171.722972972973}
							cy={0}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeBranchNode"
							cx={120.33783783783784}
							cy={25}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeBranchNode"
							cx={223.10810810810813}
							cy={25}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeBranchNode"
							cx={73.78378378378378}
							cy={50}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeBranchNode"
							cx={166.8918918918919}
							cy={50}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeLeaf"
							cx={205.54054054054058}
							cy={50}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeLeaf"
							cx={240.6756756756757}
							cy={50}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeBranchNode"
							cx={36.89189189189189}
							cy={75}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeBranchNode"
							cx={110.67567567567568}
							cy={75}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeLeaf"
							cx={149.32432432432432}
							cy={75}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode" />
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeLeaf"
							cx={19.32432432432432}
							cy={100}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeLeaf"
							cx={54.45945945945945}
							cy={100}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode">
						<circle
							className="binaryTreeLeaf"
							cx={93.10810810810811}
							cy={100}
							r={7}
						/>
					</g>
					<g className="binaryTreeNode" />
					<g className="binaryTreeTextGroup">
						<g className="binaryTreeBranchText">
							<text
								fontFamily="system-ui"
								x={171.722972972973}
								y={0}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"1"}
							</text>
						</g>
						<g className="binaryTreeBranchText">
							<text
								fontFamily="system-ui"
								x={120.33783783783784}
								y={25}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"2"}
							</text>
						</g>
						<g className="binaryTreeBranchText">
							<text
								fontFamily="system-ui"
								x={223.10810810810813}
								y={25}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"3"}
							</text>
						</g>
						<g className="binaryTreeBranchText">
							<text
								fontFamily="system-ui"
								x={73.78378378378378}
								y={50}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"4"}
							</text>
						</g>
						<g className="binaryTreeBranchText">
							<text
								fontFamily="system-ui"
								x={166.8918918918919}
								y={50}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"5"}
							</text>
						</g>
						<g className="binaryTreeLeafText">
							<text
								fontFamily="system-ui"
								x={205.54054054054058}
								y={50}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"7"}
							</text>
						</g>
						<g className="binaryTreeLeafText">
							<text
								fontFamily="system-ui"
								x={240.6756756756757}
								y={50}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"8"}
							</text>
						</g>
						<g className="binaryTreeBranchText">
							<text
								fontFamily="system-ui"
								x={36.89189189189189}
								y={75}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"11"}
							</text>
						</g>
						<g className="binaryTreeBranchText">
							<text
								fontFamily="system-ui"
								x={110.67567567567568}
								y={75}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"17"}
							</text>
						</g>
						<g className="binaryTreeLeafText">
							<text
								fontFamily="system-ui"
								x={149.32432432432432}
								y={75}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"22"}
							</text>
						</g>
						<g className="binaryTreeLeafText">
							<text
								fontFamily="system-ui"
								x={19.32432432432432}
								y={100}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"32"}
							</text>
						</g>
						<g className="binaryTreeLeafText">
							<text
								fontFamily="system-ui"
								x={54.45945945945945}
								y={100}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"28"}
							</text>
						</g>
						<g className="binaryTreeLeafText">
							<text
								fontFamily="system-ui"
								x={93.10810810810811}
								y={100}
								dy="0.3em"
								opacity={1}
								textAnchor="middle"
								fill="black"
								fontSize="0.55rem">
								{"19"}
							</text>
						</g>
					</g>
				</g>
			</g>
		</svg>
	</div>
</div>

The pages below contain notes from various computer science courses I've
taken, as well as my own independent readings. This page is updated daily,
but every now and then I will miss something. I try to correct these errors
during a full-weekend review at month's end. In the meantime, please excuse
errors or omissions.

Separately, I have another page, Review of Mathematics, containing similar
notes for mathematics. I hope at least one of these pages helps anyone else
exploring computer science.

The notes are organized by volume (indicated by a Roman numeral), then by
chapter (indicated by a Hindu-Arabic numeral). The organization attempts to
mirror a typical course sequence for a computer science concentration at a
U.S. university. The volumes focus on a specific area of computer science,
and the chapters focus on topics within that area.

Of note, the volumes are named in the grammar:

$$
	\textit{<topic>}: \textit{<language>}
$$

This is _not_ intended to communicate some notion that the topic offers notes on
${\ltn language \gtn.}$ Instead, it's intended to communicate that the
predominant language used for illustration is ${\ltn language \gtn.}$ In fact,
most of the notes in _Review of Computer Science_ do not cover the nuances of
${\ltn language \gtn.}$ Numerous other materials achieve this task, and theyi
achieve it well.

Importantly, the volumes increase in difficulty by Roman numeral.
Difficulty in terms of implicit assumptions about the reader's knowledge.
For example, in the later materials on Assembly programming, I do not
explain what a variable is. I assume readers are familiar with for-loops if
they're going so far as to examine the details behind Assembly. I think
that's a fair assumption to make.

Volumes with higher roman numerals, however, have much more details on a
particular topic. A common practice in computer science education is to
hide a great deal of the details in the early course. In fact, any good
computer science course will structure itself after identifying the
following levels of detail:

1. must know,
2. should know,
3. nice to know,
4. edge cases,
5. trivial

Some Conventions. In my experience, English is not the best language for
communicating ideas in computer science. There are many concepts in
computer science that can easily come into conflict with conversational and
formal English. As such, please take note of the following conventions I've
adopted in my writing.

First, I use the words to and through to mean different things in the
context of intervals. I adopt this convention because there's no
consistency in English when it comes to counting. My remedy: The phrase “1
to 3” denotes the open interval $[1, 3)$. In other words, when I write
“Count from 1 to 3,” I mean, “1, 2.” The phrase “1 through 3” denotes the
closed interval $[1,3]$. I.e., the phrase “Count from 1 through 3” implies
“1, 2, 3.” In short, “to” means exclude the last number, and “through”
means include the last number.

Second, in formal American English, punctuation at the end of a clause is
inserted inside quotation marks, if any. For example: This is “correct,”
But not “this”. This is one of the more asinine rules of American English
that does not appear to serve any purpose. Worse, it comes into conflict
with sentences refering to string values. To avoid any confusion, I adopt
the more sensible British rule, where punctuation is placed “outside”.
Owing to how deeply ingrained the American rule is in me, there may be
instances where I succumb to the habit, but never will I do so with string
values.

Third, a few notes on logic. I will occasionally use the clipping “iff” to
mean “if and only if”. The biconditional is one of the most important and
pervasive constructs in logic, and it would be inefficient to have to write
“if and only if” every time. Additionally: The word “or” is always the
inclusive-or, and where the exclusive-or is intended, I use either “a xor
b” or “a or b but not both”. Finally, the construct “x unless y” means “If
y, not x”.

Fourth, I've no qualms with contractions. You'll find I use them liberally,
but I will occassionally refrain for clarity and flow. I personally do not
think writing is any less serious or communicative if written without the
use of contractions. In fact, strict adherence to contraction abstinence
leads to rigid and distant communications. I'm not a fan.

Finally, having gone through the rigidity of a legal education and the
crucible of law review, I've learned not to fight the evolution of
language. In fact, one of the wonders of computer science is seeing how
programming languages evolve over time: some languages grow more efficient
at communicating and expressing ideas, others demonstrate what not to do
when expressing things, and yet others exhibit the sprouts for something
very clever. In writing these materials, I err on the side of promoting
change in the English language.
