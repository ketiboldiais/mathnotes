...
<Head>
	<title>Intro to Trees</title>
	<meta name={`description`} content={`Notes on trees.`}/>
</Head>

# Introduction to Trees

This chapter covers notes on _trees_.

<div className={`outline`}>

1. [Tree Relations](#tree-relations)
...
11. [k-ary Trees](#k-ary-trees)
	1. [Strict k-ary Trees](#strict-k-ary-trees)
	2. [Strict k-ary Shrub Sizes](#strict-k-ary-shrub-sizes)
	3. [Strict k-ary Timber Sizes](#strict-k-ary-timber-sizes)
	4. [Strict k-ary Shrub Heights](#strict-k-ary-shrub-heights)
	5. [Strict k-ary Timber Heights](#strict-k-ary-timber-heights)
	6. [Relationship: Leaves & Branch Nodes](#relationship-leaves--branch-nodes)

</div>

In this section, we examine the _trees_. Trees are a fundamental data structure in computer science. They're used as auxiliary data structures for many algorithms (in particular, searching algorithms), as well as a primary data structure for organizing data. As we'll see throughout the materials, there are many kinds of trees—binary trees, AVL trees, M-way trees, B-trees, and so on. The materials we present in this section are general propositions regarding trees. As such, there is little to no code for this specific section. Instead, much of the content pertains to key properties and ideas that apply to _all_ trees.

<div className={`float-right`}>
	<Tree data={[[1, ""], [2, 1], [3, 1], [4, 1], [5, 2], [6, 2], [10, 6], [11,
	6], [13, 6], [7, 4], [8, 4], [9, 4], [12, 8], [14, 12], [15, 12], ]}
	edgeLength={100} scale={100} width={200} height={130}/>
</div>

Roughly speaking, a _tree_ is a collection of __nodes__ and __edges__; the nodes
being the colored circles, and the edges being the numbered lines. The nodes
need not be occupied by number, nor do the edges have to be numbered. More generally, a tree is simply a particular type of graph.
...
__tree (graph theoretic).__ Let ${\Tt}$ be the nonempty graph ${\set{\Nn, \Ee,}}$ where ${\Nn}$ is the set of all nodes on ${\Tt,}$ and ${\Ee}$ is the set of all edges. Then ${\Tt}$ is a _tree_ if, and only if, the following propositions are true. (1) For any two nodes ${x,y \in \Nn,}$ there exists an edge ${(x,y) \in Ee.}$ (2) ${\Tt}$ is _acyclic_.
...
__tree (set theoretic).__ Let ${\Nn}$ be a set of nodes and ${\preceq}$ a partial ordering on ${\Nn.}$ Then the poset ${(\Nn,\preceq)}$ is a _tree_ if, and only if, the following propositions are true. (1) ${\preceq}$ is a wellfounded partial ordering. (2) For every ${n \in \Nn,}$ the set ${\set{m \in \Nn : m \prec n}}$ is well-ordered by ${\preceq.}$ (3) For all ${m,n \in \Nn}$ there exists a ${v \in \Tt}$ such that ${v \preceq m}$ and ${v \preceq n.}$
...
get ${n-1}$ edges because the very first node, called the __root__, has no
__parents__ or __ancestors__.

First, the total number of nodes in a tree is called the tree's __order__,
...
A __root__ is a node with at least one edge to a node below it, but no
...
called a __parent.__ A __child__ is a node with at least one edge to a node
...
A node with one child is called a __uniparous node__. A node with two
children is called a __biparous__ node. A node with three children, a
__triparous node__, and so on.

A node with no children is called a __leaf__. Any node that is not a
leaf—i.e., a node with children—is called a __branch node__. In the tree
diagrams we've seen thus far, branch nodes have been colored beige, and
leaves have been colored green.

The number of leaves in a tree is called the __foliage count__, denoted
...
an __ancestor__ of ${n_i,}$ and ${n_i}$ is a __descendant__ of ${n_0.}$ In
...
node ${n_i,}$ we use the term __strict descendants__, denoting such set
${I(n_i).}$[^issues_note] To refer to all the ancestors of ${n_i,}$ we use
the term __strict ancestors__, and denote the set ${A(n_i).}$ For example,
...
A __sibling__ is a node that shares a parent with another node. For
...
The nodes ${n_h,}$ ${n_i,}$ and ${n_j,}$ however, are __cousins__—nodes
whose parents are siblings. We also say that ${n_e,}$ the parent of
${n_j,}$ is a __pibling__ of ${n_h}$ and ${n_i,}$ and that ${n_h}$ and
${n_i}$ are __niblings__ of ${n_j.}$

A node with no siblings is called a __monoprogeny__, or __monoprog__. In
...
The __degree__ of a node ${n_i}$ is the number of the ${n_i}$'s _children_
...
The __degree of a tree__, or __tree degree__, has varying definitions. Some
...
the __tree degree__ as follows:

> __definition__ The _degree of a tree_ ${T}$ denoted ${k,}$ is the maximum
...
A __path__ is essentially a "route" or "road" from some node to another
...
${n.}$ The length of this path is called the __depth__ of ${n.}$ Because
...
The __tree depth__ ${D}$ is the maximum depth among all nodes in the tree.
In the tree above, the tree depth is ${3.}$

Each node ${n}$ in the tree also has a __height__, the number of edges on
...
The __tree height__ ${H}$ is the maximum height among all nodes in the
tree. In the tree above, the tree height is ${3.}$

## Binary Trees

The __binary tree__ is a tree of degree-${2}$—a tree where every node can
...
The binary tree above is an example of a __perfect binary tree__—a binary
...
<BinaryTree data={[[1, [2, null]]]} edgeLength={100} scale={60} />
...
a __degenerate binary tree__ or a __pathological binary tree__. When the
degenerate tree skews left, we call it a __left-skewed tree__.
...
And when the tree skews right, we call it a __right-skewed tree__:
...
> __lemma__. Given ${|N|}$ nodes, the number of possible unordered trees,
...
> __lemma__. Given ${|N|}$ nodes, the number of possible ordered trees,
...
called an __unordered tree permutation of order ${|N|}$ (${N}$-UTP)__.
...
> __definition__. Given ${|N|}$ nodes, the number of unordered binary tree
...
> __definition__ The shortest possible tree that can be achieved with ${|N|}$
...
> __definition__ The tallest possible tree that can be achieved with ${|N|}$
...
> __definition__ Given a height ${H,}$ the minimum order necessary to
...
> __definition__ Given a height ${H,}$ the maximum order necessary to
...
> __definition__ The minimum height achieved with ${|N|}$ nodes is called the
...
> __definition__. A lumber ${L}$ is a tree with the greatest possible height
> ${H(L)}$ given ${n}$ nodes.
...
> __definition__. A shrub ${S}$ is a tree with the least possible height
> ${H(S)}$ given ${n}$ nodes.
...
Recall that a __leaf node__ is a node with no children; i.e., a node with
...
__twig__, and a branch node with ${2}$ children a __bough__. Is there a
...
> __lemma__. Given a binary tree ${B}$ with leaves ${\ell}$ and boughs ${b,}$
...
Recall that a __binary tree__ is a tree where each node has at most two
...
referring to a __rooted binary tree__—a tree data structure with: (1) a
...
A __proper binary tree__ is a binary tree where each node has either ${0}$
...
[^proper_binary_tree_note]:
    Proper binary trees are also called __full binary trees__ or **plane
    binary trees**.
...
__proper lumber__—the tallest possible binary tree given ${n}$ nodes:
...
A __complete binary tree__ is a binary tree where each generation other
...
non-green trees are __incomplete binary trees__—binary trees that are not
complete.
...
the non-green trees are __imperfect binary trees__— trees that are not
...
Balanced binary trees come in two forms: __height-balanced trees__ and
__weight-balanced trees__. Below are the definitions.
...
Binary trees are more generally called k-ary trees. Specifically, they are
a 2-ary tree.

> __definition__: A k-ary tree is a rooted tree where each node has at most
> ${k}$ children.
...
A __ternary tree__ is a 3-ary tree, so ${k=3.}$ Thus, in a ternary tree,
each node can only have ${0,}$ ${1,}$ ${2,}$ or ${3}$ children. For
example, the green trees below are all ternary trees, but the red tree is
not:
...
Similarly, a 4-ary tree is a tree where each node only has either ${0,}$
${1,}$ ${2,}$ ${3,}$ or ${4}$ children.

### Strict k-ary Trees

A _strict k-ary tree_ is a k-ary tree where each node has either (1) zero
children or (2) exactly ${k}$ children.

> __definition__. Let ${T}$ be a k-ary tree. ${T}$ is a _strict k-ary tree_
> iff every node in ${T}$ has a ${0}$ or ${k}$ children.

Thus, a __strict binary tree__ (what we referred to earlier as a _proper
binary tree_) is a binary tree where every node has either ${0}$ or ${2}$
children. A __strict ternary tree__ is a ternary tree where every node has
either ${0}$ or ${3}$ children. A __strict 4-ary tree__ is a 4-ary tree
where every node has either ${0}$ or ${4}$ children. And so on.

### Strict k-ary Shrub Sizes

The shrub size lemmas we saw earlier apply to strict k-ary trees.

> _Lemma: Strict ${k}$-ary Shrub Sizes_. Let ${H \in \uint+}$ and
> ${k \in \uint+.}$ The minimum number of nodes, i.e., the _minimum size_,
> necessary to achieving a k-ary tree of height ${H}$ is the _strict
> ${k}$-ary shrub size_, denoted ${S_{min}^k(H),}$ where:
>
> $$
> 	  S_{min}^k(H) = kH + 1
> $$
...
> _Lemma: Strict ${k}$-ary Shrub Sizes_. Let ${H \in \uint+}$ and
> ${k \in \uint+.}$ The maximum number of nodes, i.e., the _maximum size_,
> necessary to achieving a k-ary tree of height ${H}$ is the _strict
> ${k}$-ary timber size_, denoted ${S_{max}^k(H),}$ where:
>
> $$
...
The strict k-ary lumber size lemma yields the strict k-ary shrub height
lemma: The smallest possible height strict k-ary tree given ${N}$ nodes.
...
> _Lemma: Strict ${k}$-ary Shrub Height_. Let ${N, k \in \pint.}$ Given
> ${N}$ nodes, the shortest possible height strict k-ary tree, called the
> _${k}$-ary shrub_, has the height ${H_{min}^k(N),}$ called the _${k}$-ary
> shrub height_, where:
>
> $$
...
By a similar line of reasoning, we can derive the strict k-ary timber
height lemma from the strict k-ary shrub size lemma: The _tallest_ possible
height strict k-ary tree given ${N}$ nodes.
...
> _Lemma: Strict ${k}$-ary Timber Height_. Let ${N, k \in \pint.}$ Given
> ${N}$ nodes, the tallest possible height strict k-ary tree, called the
> _${k}$-ary timber_, has the height ${H_{max}^k(N),}$ called the
> _${k}$-ary timber height_, where:
>
> $$
...
> __lemma__. Let ${k, \ell, b \in \pint,}$ where ${\ell}$ is the number of
> leaves and ${b}$ is the number of branch nodes in a strict k-ary tree.
> Then:
>
> $$
> 	\ell = (k - 1)b + 1
