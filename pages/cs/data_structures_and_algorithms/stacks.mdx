...
The following materials pertain to the __stack data structure__. Unlike the
...
the stack, this behavioral principle is __Last-in-first-out (LIFO)__.
...
1. When we place an object ${x}$ on the stack, we are __pushing__ ${x}$ on
   the stack.

2. When we take an object ${x}$ off the stack, we are __popping__ ${x}$ off
   the stack.
...
The variable ${F}$ indicates a __frame__—an element of the stack—and the
variable ${t}$ is a pointer to the __topmost frame__ (in the diagram, the
frame colored green). In these materials, we also refer to the topmost
frame as the __T-frame__, and all others as simply _frames_. Each frame has
...
A __static stack__ is a stack with a fixed capacity—the maximum number of
...
fixed __capacity__. And where the array has a variable _length_, the static
stack has a variable __height__.
...
<Sequence data={[0, 0, 0, 0]} scale={40} />
...
<Sequence data={[2, 0, 0, 0]} scale={40} />
...
<Sequence data={[2, 9, 0, 0]} scale={40} />
...
<Sequence data={[2, 9, 1, 0]} scale={40} />
...
<Sequence data={[2, 9, 1, 8]} scale={40} />

At this point, we say that the stack is __full__—the stack's height equals
...
<Sequence data={[2, 9, 1, { val: 8, focus: true }]} scale={40} />
...
The first fundamental operation to all stacks is __pushing__. The
...
`capacity`. Accordingly, we need a __capacity guard__.

## Capacity Guard

With static stacks, we must always be using a __capacity guard__—a
...
The __empty guard__ is a procedure that determines if a given stack is
...
When we __pull__ from a stack, we are retrieving the data value in the
...
In contrast to static stacks, __dynamic stacks__ are stacks that have no
