...
where ${n \in \ZZ^{+}.}$ From number theory, this implies that the number
...
where ${n \in \ZZ^{+}.}$ Given that ${n}$ is a positive integer, the closure
property of the positive integers implies that the index can be expressed
as:

<figure>$$ 2k $$</figure>

where ${k \in \ZZ^{+}.}$ And again from number theory, this implies that the
