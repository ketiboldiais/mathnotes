...
So far, much of the data we've used as examples have been simple — `char`s,
...
Both DRAM and SRAM are volatile _volatile_ — there must be a constant
...
with a technique called _prefetching_: Using two separate pipelines 𝐴 and 𝐵
(called _data eyes_) from the memory cells, 1 bit is sent towards the data
queue through 𝐴, and 1 bit through 𝐵. The bits are then released to the bus
as usual — only this time, 1 bit is sent on the leading edge, and 1 bit is
sent on the falling edge. DDR SDRAM is referred to as a _2𝑛-prefetch
...
(4𝑛 prefetch), DDR3 and DDR4 are both 8 bits (8𝑛 prefetch; the DDR4 focused
...
__Read-only Memory (ROM)__ chip. The ROM is where machine level
...
1. __Programmable Read-only Memory (PROM)__ PROM is a type of ROM stores
...
2. __Eraseable PROM (EPROM)__ is PROM that can be re-programmed by erasing
   the entire chip with ultra violate light or x-rays. Moden computers have
   replaced EPROM with EEPROM.
3. __EEPROM__ is PROM that can be re-programmed through electronic erasing.
   Modern computers use EEPROM chips. EEPROM chips themselves have two
   types: (1) __electrically alterable read-only memory (EAROM)__ and (2)
   __flash memory__. The primary difference between the two is how quickly
...
All HDDs have a _disk capacity_ — the maximum number of bits that can be
...
3. _Areal density_. Measured in ${\text{bits}/\text{in}^2}$, this is the
...
We can think of the _I/O bridge_ as the system's King's Cross station —
...
computer system 𝑆, the graphics adapter (for managing image output to a
...
For example, say we wanted to read some piece of data 𝐷 from our hard disk.
...
the bits and reads the disk's sector containing 𝐷. Once it retrieves 𝐷, it
performs a _direct memory access (DMA)_ to transfer 𝐷 into main memory.
...
computations while it waits for the data 𝐷 to get to main memory. Once 𝐷 is
...
guided by the principle of __locality__:

> __locality principle__. A program 𝑃, more often than not, will use data
...
A __cache__ is a memory device used to temporarily hold a small amount of
...
Caches are a direct application of locality. Say a program 𝑃 runs, and it
...
5. The process continues until we're at the processor's _word-size_. For
   the x86-64, this is 64 bits — 8 bytes.
...
that page, a copy of it is cached in the next level's memory device — disk
memory.
...
If it's scenario (1), then we have a __cache hit__. The system reads the
data directly.

If it's scenario (2), we have a __cache miss__ — the system must fetch the
...
2. _Capacity misses_. These misses occur when the _working set_'s size
   ...
   structures, we'll more than likely hit capacity misses — they're far too
...
3. _Conflict misses_. These misses occur because the cache is large enough
   ...
   particular area 𝐴 of the memory device in the level below it. Thus, any
   data not stored in 𝐴 will result in a miss. This is restriction is
   generally found in the highest rungs of the memory hierarchy — the
...
This is the simplest approach to implementing a cache — _direct mapping_.
...
A _cache_ consists of memory cells grouped into __lines__. The lines are
then grouped into __sets__:

| set        | line 0     | line 1     | ...        | line 𝑛     |
| ---------- | ---------- | ---------- | ---------- | ---------- |
...
| 𝑛          | 0000 0000  | 0000 0000  | ...        | 0000 0000  |
...
1. 𝐵 bytes, where ${B \in \nat.}$ These bytes are where the actual data are
   stored.
2. A set of _tag_ bits that identify the block's memory address.
3. A _valid bit_ — a single bit to indicate whether the line is valid.
...
main memory address 𝐴.

Upon receipt, the CPU sends the bits comprising 𝐴 to cache memory. If the
cache doesn't have the data inside address 𝐴, it will fetch the contents of
𝐴 from main memory, and store them in the cache. But if the cache does have
a copy of 𝐴's contents, it will send 𝐴 back to the CPU.

The memory device can determine whether 𝐴's contents already exist
...
Each block has the address format above: 𝑡 bits for the tag, 𝑠 bits for the
set's index in the line, and 𝑏 bits for the block's offset (equal to the
number of 𝐵 bytes the block can store). To determine if 𝐴 exists in the
block, all the cache memory device has to do is:

1. Look at the the 𝐴 address's set index bits. This tells the cache which
   _set_ the address 𝐴's mapped cell might exist in.
2. Go to the set.
3. Look at the tag bits in the 𝐴 address. This tells the cache which _line_
   within the set the address 𝐴 might be mapped to. If the tag bits don't
   yield the 𝐴 address, then we have a cache miss: the data to be retrieved
   from address 𝐴 in main memory, brought to the cache, the line's
   occupants are evicted, and data retrieved from 𝐴 becomes the new
...
because of spatial locality — the data are all close together.
...
__stack__. Importantly, there's no guarantee that the arrays are allocated
...
2. Store each element in the nested array _column by column_ (the
   __column-major ordering approach__).
...
> _Lemma_. Let 𝐴 be a nested array, where each array in 𝐴 contains 𝑁
> elements of type 𝑇. Then:
>
> $$
>   \texttt{𝐴[i] = 𝐴 + (i ⨉ 𝑁 ⨉ 𝑆(𝑇))}
> $$
...
plain row-major ordering — we have to do two memory reads.
...
reads up to ${4 \times \texttt{idx}.}$ This results in the Assembly code:
...
> _Rule_. If a primitive data type requires 𝑘 bytes, then the address of an
> instance of that data type must be a multiple of 𝑘.
...
In the diagram above, the cells containing 𝑝 are called _padding bytes_. By
...
Why go through this trouble? Because it's far more expensive — efficiency
wise — to offset pointers with values that are not multiples of the
...
data type determines the largest alignment 𝑘. If the struct, without
padding, ends on an address that is _not_ a multiple of 𝑘, then the
compiler will insert padding to ensure the struct ends on an address that
is a multiple of 𝑘. Thus, in the diagram above, we see that the struct ends
at `p+24`, which is a multiple of 8. This corresponds to ${𝑘 = 8,}$ the
...
- The _heap_ provides memory we manually request. In C, this is done with
  ...
  `new`.
- The _data_ region is where statically allocated data is stored — global
...
security vulnerability called __buffer overflow__. Undoubtedly, buffer
...
the 8MB limit. This kind of attack is often called __stack smashing__.
...
and storing characters — there's no check against how large that input is.
...
That `char` array — the buffer in question — can only store 4 characters.
