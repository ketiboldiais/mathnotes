...
<head>
	...
	<description>Notes on networks.</description>
</head>
...
6. **Voice over Internet Protocol (VoIP)** is a UDP protocol for making voice
...
telephony_. Voice signals are tiny — just a little less than ${10\tt{kHz}.}$
...
typical webpage today hovers around ${2\tt{MB}}$ (and they're getting bigger).
...
$$
	\eqs{
		&\B_r &:= &~~\tt{link data rate}~~(\tt{bits/s}) \\
		&\L &:= &~~\tt{packet length}~~(\tt{bits}) \\
		&\A_{\tt{avg}} &:= &~~ \tt{average packet arrival rate}~~(\tt{bits $\times$ packets/s})
	}
$$

then a router's traffic intensity, called _congestion_ and denoted
${\I_{\tt{traf}},}$ is given by the formula:

$$
	\I_{\tt{traf}} = \dfrac{\L \cdot \A_{\tt{avg}}}{ \B_r }
$$
...
${\I_{\tt{traf}},}$ and the ${y}$-axis corresponds to the average queueing
delay. Based on the model, we can infer the following: 

1. When ${\I_{\tt{traf}} \gtrsim 0,}$ the average queueing delay is small.
2. When ${\I_{\tt{traf}} \lesssim 1,}$ the average queueing delay is large.
3. When ${\I_{\tt{traf}} \gtn 1,}$ the average delay is infinite.
...
time it takes to place all ${\L}$ bits onto the link, denoted ${\tt{TD}}$ is
defined as:
...
to travel from one end of the link to the other, denoted ${\tt{PD},}$ is given
by the formula:
...
$$
	\eqs{
		& \d_{\tt{proc}} & := &~~\tt{processing delay} \\
		& \d_{\tt{queue}} & := &~~\tt{queueing delay} \\
		& \d_{\tt{trans}} & := &~~\tt{transmission delay} \\
		& \d_{\tt{prop}} & := &~~\tt{transmission delay} \\
		& \h & := &~~\tt{number of hops} \\
	}
$$

then the total delay it takes for a packet to travel from one node to a
subsequent node, denoted ${\d_{\tt{nodal}},}$ is defined as:
...
${100\m,}$ and speeds cap at about ${3 \tt{Mbps.}}$ Bluetooth's primary
...
$$
	\C = \B \cdot \lg \paren{1 + \dfrac{\P}{\tt{N}}}
$$

where ${\B}$ is the bandwidth, ${\P}$ is the average signal power, and
${\tt{N}}$ is the average noise power. The term ${\P / \tt{N}}$ is often called
the _signal-to-noise ratio_, and is measured in decibels (dB). This term can be
expressed with the formula:

$$
	10 \cdot \log_{10}(\P/\tt{N})
$$


For example, a telephone line with ${\P/\tt{N} = 30\tt{dB}}$ and an audio
bandwidth of ${3\tt{kHz}}$ has a maximum data rate of: of:

$$
	\C = 3000 \cdot \lg{1001} \approx 30 \tt{kbps}
$$
...
lot we can do about noise — the ${\tt{N}}$ term — aside from shielding our
...
_bitrate_, and we quantify it in terms of bits per second (${\tt{bps}}$). This
leads to the following units:

| Unit            | Meaning                       |
| --------------- | ----------------------------- |
| ${1~\tt{bps}}$  | ${1}$ bit per second          |
| ${1~\tt{kbps}}$ | ${1~000}$ bits per second     |
| ${1~\tt{Mbps}}$ | ${1~000}$ kilobits per second |
| ${1~\tt{Gbps}}$ | ${1~000}$ megabits per second |
| ${1~\tt{Tbps}}$ | ${1~000}$ gigabits per second |

Alternatively, we can also measure bitrates in terms of bytes:

| Unit            | Meaning                                                                                                                                                                                   |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ${1~\tt{Bps}}$  | ${8}$ bits per second                                                                                                                                                                     |
| ${1~\tt{KBps}}$ | ${8~000}$ bits per second, ${1~000}$ bytes per second                                                                                                                                     |
| ${1~\tt{MBps}}$ | ${8~000~000}$ bits per second, ${1~000~000}$ bytes per second, ${1~000}$ kilobytes per second                                                                                             |
| ${1~\tt{GBps}}$ | ${8~000~000~000}$ bits per second, ${1~000~000~000}$ bytes per second, ${1~000~000}$ kilobytes per second, ${1~000}$ megabytes per second                                                 |
| ${1~\tt{TBps}}$ | ${8~000~000~000~000}$ bits per second, ${1~000~000~000~000}$ bytes per second, ${1~000~000~000}$ kilobytes per second, ${1~000~000}$ megabytes per second, ${1~000}$ gigabytes per second |
