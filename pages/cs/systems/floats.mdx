<head>
	...
	<description>Notes on floating point numbers.</description>
</head>
...
One method is **fixed point representation**. The idea behind fixed point
...
\begin{aligned}
	...
	&2~990.0~~~\tt{3 places} \\
	&29~900.0~~~\tt{4 places} \\
	&299~000.0~~~\tt{5 places} \\
	&2~990~000.0~~~\tt{6 places} \\
	&29~900~000.0~~~\tt{7 places} \\
	&299~000~000.0~~~\tt{8 places} \\
\end{aligned}
$$
...
We call ${n}$ the **exponent**, and ${m}$ the **mantissa** (or
**significand**). At this point, we can state the following proposition:

> **proposition.** Given a representation of the form ${m \times 10^n,}$ the
...
**Compute the Fractionals as Integers, then Convert to Floats.** This is a
...
**Turn off the Code Formatter and Linter.** Code formatters (e.g.,
