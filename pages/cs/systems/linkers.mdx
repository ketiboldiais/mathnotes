...
1. **Static linking**. In creating the executable, the linker will ensure
...
2. **Dynamic linking.** The executable files don't contain any library
...
1. **Relocatable object files**. These object files are identified with the
...
2. **Executable object file.** These object files are identified as `a.out`
...
3. **Shared object file**. These are a special kind of relocatable object
...
The array of `nop` instructions is called a **nop sled**, and it's used to
...
1. **Symbol resolution.** For each file, resolve all symbols.
2. **Merging.** For each file, merge all of the instructions and data into
   single sections.
3. **Relocation.** Relocate the sections into their final absolute memory
...
**symbols**. Because each source code file results in its own object file,
...
1. **Global symbols**. These are identifiers defined in 𝐴 that can be
...
2. **External symbols.** These are identifiers used in 𝐴 or 𝐵 that are
...
3. **Local symbols**. These are identifiers defined globally in 𝐴 that can
...
and _resolve_ the **symbol references**: an identifier associated with a
...
1. The **symbol offset** contains the offset values for each symbol. We can
...
2. The **binding** indicates whether the symbol is a global, static, or
   reference.
3. The **type** indicates whether the symbol is a function (`F`), object
   (`O`), or undefined (`_UND_`).
4. The **section** indicates which section the symbol belongs to. In this
   case, we see that it's either the _text_ section, or the _data section_.
5. Finally, the **name** field provides the symbol's name.
