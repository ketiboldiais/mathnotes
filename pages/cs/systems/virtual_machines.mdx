...
question is the __virtual machine__. The general process works as follows:
...
approach? Sure. The programmer could just write a __virtual machine__ — a
...
This approach is called __two-tier compiling__ — compilation consisting of
...
into __bytecode__ — virtual machine code. A virtual machine is simply an
...
called a __virtual machine (VM) implementation__. In short, each computer
...
entities — is called __virtualization__.
...
1. A __goto command__ to execute an instruction other than the next
   instruction,
2. An __ifgoto command__ to execute an instruction other than the next, but
   only if a certain condition is mset,
3. a __label command__ to "bookmark" particular instructions.

In low level programs like the vitual machine's, branching commands fall
under two broad categories: (1) __unconditional branching__ and (2)
__conditional branching__.
...
1. A __call command__ for calling functions.
2. A __function command__ for indicating functions.
3. A __return command__ to route the function's output elsewhere.
...
This structure has a specific __call chain__ or __call sequence__:

$$
	\texttt{f} \to \texttt{g} \to \texttt{h}
$$

Every single one of the functions above has some __state__ — private worlds
...
__caller__), and a state for `multiply` (the __callee__).
...
called the caller's __function frame__.
...
__global stack__. Examining the preceding example, we can imagine a
...
abstraction called the __call stack__:
...
set of __protocols__, or __contracts__ — a set of rules and conditions that
the entities performing the operation must always abide by. For functions,
these entities are the __caller__ and the __callee__. For the caller, these
rules apply:
...
$$
	\texttt{call}~\ix{\it{function-name}}~\ix{\it{n-args}}
$$
...
$$
	\texttt{function}~\ix{\it{functionName}}~\ix{\it{n-vars}}
$$
...
1. A __substantive interpretation__ — the actual, numeric values `257` and
   `1024`, and
2. a __procedural interpretation__ — the memory addresses `257` and `1024`.
...
store all of these static variables in a __global space__ — a designated
...
- A __parser__ (named `Parser`) that parses each VM command into lexical
  elements. This is the program that breaks the stream of virtual machine
  code into smaller, meaningful parts.
- An __encoder__ (named `CodeWriter`) that will take and implement the
  parser's output.
- A __driver__ (a program called `Main`) that will conduct and direct the
