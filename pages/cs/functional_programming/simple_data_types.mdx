...
and concepts. In ML, a program is a sequence of **bindings**. Each
**binding** has a **type**, which is checked. If it the binding passes its
type-check, then it is **evaluated**.

A binding's type depends on its **static environment** &mdash; roughly, the
_types_ of the preceding bindings. How a binding is evaluated depends on
its **dynamic environment** &mdash; roughly, the _values_ of the preceding
bindings. The term **environment** usually refers to the dynamic
environment, and the term **context** usually refers to the static
...
**variable binding**. It takes the general form:
...
Every binding in ML has a**scope**, which we can think of as &#8220;where
...
What is a **value**? A*value* is an expression that cannot be further
...
A **recursive** function is a function that calls itself. Because the
function calls itself, we must always have a **base case** &mdash; a call
to the function that immediately returns a value, ending the line of calls
made. The recursive function also requires an **inductive case** &mdash;
...
The function above is an example of **linear recursion**: Each recursive
...
In **nonlinear recursion**, the recursion involves more than one recursive
call.
...
in ML is through **pairs**. To build a pair, we use the following syntax:
...
Tuples are of fixed but **arbtirary arity** &mdash; we can nest tuples
...
provides **lists**. A list containing ${n}$ values is initialized with the
following syntax:

$$
	\mm{[$v_1$, $v_2$, \ldots, $v_n$]}
$$

Note that unlike tuples, lists are **homogeneous** &mdash; a list's members
...
$$
	\mm{let $b_1$, $b_2$, \dots $b_n$ in $e$ end}
$$
...
1. Is `x_list` the empty list?
   - Yes ${\implies}$ evaluate to `0`.
   - No ${\implies}$ ask the next question.
2. Is the tail of `x_list` the empty list? (i.e., Is the list a singleton
   list?)
   - Yes ${\implies}$ evaluate to the head of `x_list`.
   - No ${\implies}$ ask the next question.
...
the **exponential blowup** &mdash; `max()` asks for `max()` twice, and in
...
**Syntax errors** are errors caused by writing something incorrectly.
...
**Runtime errors** are errors that occur during evaluation. This might be
...
Errors are not caught by ML can be broadly classified as **logic errors**.
