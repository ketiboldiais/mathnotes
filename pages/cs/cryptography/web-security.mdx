...
4. When the user makes later requests (e.g., ~delete~, ~post~, ~put~, etc.), the server can verify that the user is who they claim to be by:
	1. Reading the ${\mo{sessionId}}$ value ${n}$ in the cookie:
	$$
		\mo{cookie}\mc\mo{sessionId}
	$$
	2. Requesting the database to return the username mapped to by ${n:}$
	$$
		v = R\ix{\mo{cookie}\mc\mo{sessionId}}
	$$
	3. If ${v \neq \mo{NULL},}$ then the user is verified and the server processes the request. Otherwise, the user is not verified and the server returns a response error.
