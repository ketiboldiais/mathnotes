...
9. [Designing Servers](#designing-servers)
	1. [Warning on Express](#warning-on-express)
...
### Cookies
> __~definition: cookie~.__ A _cookie_ is a string isomorphic to ${k,v}$ where ${k}$ is a key and ${v}$ is a value. The key maps to a variable of the server process, and ${v}$ is the variable's value.
 
~example~. A web server might have a cookie field ${theme,}$ which sets the site's color theme (a color palette for styling). That field may have a default value of ${light}$ and a alternative value of ${dark.}$ If the default value is used, certain lines of CSS are served. If the user changes it to dark, different lines of CSS are served.

If a server implements cookie functionality, it sends the browser an initial cookie. When the browser sends responses, it includes these cookies for the server to use. The server then watches for PUT or POST requests that potentially modify the cookies. If these modifications are found, the server returns a modified cookie for the browser to attach on subsequent requests.
...
> __`Secure`.__ If the string ${\mo{Secure}}$ is included in the `Set-Cookie` string, cookies will only be transmitted to and from the server over HTTPS connections.
