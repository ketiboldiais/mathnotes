...
<Metadata
  ...
  description={'Notes on applications of integrations.'}
  keywords={'integration, calculus, probability'}
/>
...
The problem with all of these approaches — or, more generally, the Riemann Sum
approach — is that it's terribly inefficient in practice. Not only must we loop
...
The three points — marked above as ${y_0,}$ ${y_1,}$ and ${y_2}$ — lie on the
...
be surprising — chances are, those non-smooth functions are non-differentiable.
