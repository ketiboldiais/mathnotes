...
<head>
	...
	<description>Notes on exponential functions.</description>
</head>
...
> Definition. For any ${x \in \RR,}$ an exponential function is a function
> of the form ${f(x) = ab^x.}$ Where:
>
> - ${a \in \RR^*}$ (${\RR^* = \RR \setminus \{ 0 \}}$)
> - ${b \in \RR^{+}}$

The domain of ${f}$ is the set of all real numbers ${\RR.}$ If ${a > 0,}$
then ${\text{ran}(f) \in \RR^{+}.}$ If ${a < 0,}$ then
${\text{ran}(f) \in \RR^{-}.}$ Further:
...
> Definition. For every ${x \in \RR,}$ ${\{a, b \in \RR\ \mid b \neq 1\},}$
...
> $$
>
> where ${x \in \RR,}$ ${a, b \in \RR,}$ and ${b < 1.}$
...
> form ${G(t) = ve^{gt}}$ where ${t \in \RR,}$ ${v, g \in \RR^{+},}$ >
...
> ${D(t) = ve^{dt}}$ where ${t \in \RR,}$ ${v, r \in \RR^{+},}$ ${r < 0,}$
...
3. If the problem refers to _continuous growth_, then ${r > 0}$ (i.e.,
   ${r \in \RR^{+};}$ ${r}$ is a positive real number).
4. If the problem refers to _continuous decay_, then ${r < 0}$ (i.e.,
   ${r \in \RR^{-};}$ ${r}$ is a negative real number).
...
If ${C \in \RR^{+},}$ then ${G(f)}$ shifts _up_ the ${y \text{-axis}}$ by
${C}$ units. If ${C \in \RR^{-},}$ then ${G(f)}$ shifts _down_ the
...
the following: (i) ${\text{dom}(f) = \{ x \in \RR \};}$ i.e., the domain
...
${\text{ran} = \{ f(x) \in \RR \mid f(x) > -C \},}$ while the domain remains
unchanged, ${\{ x \in \RR \}.}$
...
${(- \infty , \infty),}$ ${\RR,}$ remains unchanged. (2) The horizontal
...
4. If ${C \in \RR^{+},}$ shift ${G(f)}$ _left_ ${C}$ units.
5. If ${C \in \RR^{-},}$ shift ${G(f)}$ _right_ ${C}$ units.
6. If ${d \in \RR^{+},}$ shift ${G(f)}$ _up_ ${d}$ units.
7. If ${d \in \RR^{-},}$ shift ${G(f)}$ _down_ ${d}$ units.
8. State the domain, ${\RR,}$ and the range, ${d, \infty,}$ and the
