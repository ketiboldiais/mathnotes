...
> ${c \in \RR.}$ Suppose ${y = f(x).}$ Substituting ${y}$ with ${y - c},$ we
> obtain ${y - c = f(x),}$ which is equivalent to ${y = f(x) + c.}$ This is
> a _vertical translation_ of ${f(x).}$
>
> If ${c \in \RR^{+},}$ then ${f(x)}$ transforms to ${f(x) + c,}$ which
> shifts ${G(f)}$ by ${c}$ units _up_ the $y$-axis.
>
> If ${c \in \RR^{-},}$ then ${f(x)}$ transforms to ${f(x) + c = f(x) - c,}$
...
> ${c \in \RR.}$ Suppose ${y = f(x).}$ Substituting ${x}$ with ${x - c},$ we
