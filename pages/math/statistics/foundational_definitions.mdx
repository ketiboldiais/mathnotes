...
1. Organizing and summarizing data. The methods for accomplishing this task
   are collectively called __descriptive statistics__.
2. Drawing conclusions from data. The methods for accomplishing this task
   are collectively called __inferential statistics__.

With this introduction, let's define some basic terms in statistics.

## Population

In statistics, data is generally collected from some __population__. A
...
limitations, we instead select a __sample__ of the population. A sample is
a subset of the larger population.

### Statistics

From the sample data, we can calculate one or more __statistics__ — some
...
All statistics are estimates of a __parameter__ of the population. A
...
whether the sample is a __representative sample__. A sample is considered
...
1. A __numerical variable__ is a variable that can be defined as a value of
   some unit. For example, weight in kilograms, height in meters, or time
   in seconds.
2. A __categorial variable__ is a variable that places a person or object
...
1. __Qualitative data__ is data resulting from categorizing or describing
...
2. __Quantitive data__ is data resulting from counting or measuring
   population characteristics. For example, from a survey of the
...
1. __Quantitative discrete data (QDD)__ is quantiative data that results
...
2. __Quantiative continuous data (QCD)__ is quantitative data that results
...
> __definition__. Let ${x}$ be some sample of population ${P.}$ ${x}$ is a
> representative sample of ${P}$ if, and only if, ${x}$ has the same
> characteristics as ${P.}$

To satisfy this definition, statisticians employ __random sampling__. In
...
The easiest method is __simple random sampling__. With this approach, we
select a __sample size__ ${n,}$ and from the population ${P,}$ we select
...
Another random sampling method is __stratified sampling__. The procedure is
as follows:

1. Divide the population into ${n}$ groups.[^strata]
2. From each group, take a _proportionate_ number of members.

[^strata]: These groups are called __strata__ (singular __stratum__).

#### Cluster Sampling

Alternatively, we can also employ __cluster sampling__ as another random
sampling approach:
...
__Systematic sampling__ is useful if we already have a list of all the
population members.

1. Randomly select a starting position 𝑖 in the list.
2. From 𝑖, select every ${n^{\text{th}}}$ member of the list.
...
1. Were there __sampling errors__? The actual process of collecting the
...
2. Were there __non-sampling errors__? Errors might be caused not by the
...
3. Was there a __sampling bias__? A sampling bias occurs when the sampling
...
> __Definition__. Let 𝑆 be a set of non-numeric values (commonly strings)
...
the data is called __dichotomous data__. For example, if the set ${S}$
...
> __Definition__. Let 𝑆 be a sorted array of ordinal values (commonly
...
> __Definition__. Let 𝐼 be an interval of ${\reals}$ _without_ a lower bound
...
$$
   [\ldots, \text{12pm}, \text{1pm},\text{2pm},\text{3pm},\text{4pm},\text{5pm}]
$$
...
${\text{12pm},}$ the statement makes no sense — _twice as late with respect
to what?_
...
> __Definition__. Let 𝐼 be an interval of ${\reals}$ _with_ a lower bound
> (called a _meaningful zero_), and where each member of 𝐼 is equidistant
> from their left and right neighbors. A datum 𝑑 is considered an _interval
> datum_ iff ${𝑑 \in 𝐼.}$
