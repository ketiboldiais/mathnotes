...
Suppose we have a number ${a}$ where ${a \in \RR^{+}.}$ The **square root**
...
> __definition__. Given ${a \in \RR^{+},}$ ${\sqrt{a}}$ is the number ${n,}$
> where ${n \in \RR}$ such that ${n^2 = a.}$
...
> Definition. Let ${a \in \RR,}$ where ${a}$ has at least one
