...
<Metadata
	title={"Functions"}
	description={"Notes on functions in discrete mathematics."}
	keywords={"Functions, discrete mathematics, mapping"}
/>
...
Above, ${A}$ is called 𝑓's **domain** and ${B}$ is 𝑓's **codomain**. The
domain is the set of all possible inputs of 𝑓, and the codomain is the set
of all possible outputs of 𝑓. Based on the diagram above, the only possible
inputs of 𝑓 are 𝑎, 𝑏, and 𝑐, and the only possible outputs of 𝑓 are 2, 4,
and 5.
...
values, 2 and 5. These two outputs comprise 𝑓's **image**. A function's
...
an onto function because one member of its codomain — the element 4 — is
...
to "fix" functions — i.e., make them easier to work with or to force them
into a particular definition — by adjusting the domain or codomain. This
...
that function is fairly restrictive — we have ints, floats, booleans, char
...
need a function that outputs Cartesian coordinates — an ordered pair of the
