...
The overarching focus in the following materials is with __proof__.
Question: What is a proof? A __proof__ is a method of ascertaining the
truth. One method of proof is a __mathematical proof__ &mdash; a logically
valid path from assumptions to conclusions, collectively forming a _sound
argument_. But there are other methods of proof. The __scientific method__
...
conclusion. A __trial__ is another method of proof. It begins with a
...
are up to. A __proposition__ is a statement that is either true or false.
...
${prime(n^2 + n + 41)}$ is called a __predicate__. A predicate is a
...
The part of the proposition ${\forall n \in \nat}$ is called the **domain of
discourse**. This tells us that the predicate ${prime(n^2 + n + 41)}$
relates only to ${n \in \nat.}$ We're not talking about the real numbers,
...
means "for all" and it is called a __quantifier__. More specifically, it is
called a __universal quantifier__.

To show that ${\forall \in \nat: prime(n^2 + n + 41)}$ is true, we have to
...
This method of verification is called the __method of exhaustion__ &mdash;
...
__rigorous__. Repeated observation is not enough. As the philosopher David
...
The proposition ${\forall n \in \nat: prime(n^2 + n + 41)}$ is false. Here is
...
This proposition is called __Euler's sum of powers conjecture__. The
legendary mathematician Leonard Euler __conjectured__ the proposition above
...
$$
	\exists a, b, c, d \in \uint^{+} : a^4 + b^4 + c^4 = d^4
$$

Here, we see a new quantifier, ${\exists.}$ This symbol means "there
exists." This is called an __existential quantifier__. Here is another
proposition:
...
In formal notation: ${\nexists x, y, z \in \uint^{+} : 313(x^3 + y^3 = z^3).}$
...
This is the famous __four-color theorem__. A mathematician named Francis
...
__proof-by-picture__ that was believed to be correct for over a decade.
...
on. This is proposition is known as __Goldbach's conjecture__, and it is
...
$$
	\forall n \in \uint: (n \geq 2 \nc n^2 \geq 4)
$$

The symbol ${\nc}$ means "implies." Thus, the proposition above reads,
"For any integer ${n,}$ if ${n \geq 2,}$ then ${n^2 \geq 4.}$" An
__implication__ ${p \nc q}$ is true if ${p}$ is false or ${q}$ is
true. Thus, if ${p}$ is true, and ${q}$ is true, then ${p \nc q}$ is
true. If ${p}$ is false and ${q}$ is true, then ${p \nc q}$ is false.
But if ${p}$ is false, then ${p \nc q}$ is true, regardless of whether
...
$$
	\forall n \in \uint: (n \geq 2 \iff n^2 \geq 4)
$$
...
proposition ${n^2 \geq 4 \nc n \geq 2}$ is false, the original
...
forms. Some propositions are called __axioms__. These are propositions that
...
This is called the __axiom of transitivity__, or the **transitive
...
A set of axioms is __consistent__ if no proposition can be proved to be
both true and false. A set of axioms is __complete__ if it can be used to
...
Godel's __incompleteness theorems__. Godel's proof is best left to a course
on logic, but the effect of this proof is profound: Perhaps _Goldbach's
conjecture_ is true, and it is impossible to prove.

__Discrete mathematics__ is the mathematics of discrete and finite
...
__logic__.[^induction] Accordingly, a strong logic background is invaluable
