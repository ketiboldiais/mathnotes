...
<Metadata
	title={"Graph Theory"}
	description={"Notes on graph theory"}
	keywords={"graph theory, discrete mathematics, computer science"}
/>
...
The nodes 𝑎 and 𝑏 are adjacent, as well as the nodes 𝑏 and 𝑐. But, the
nodes 𝑎 and 𝑐 are not adjacent. Closely related is the notion of
**incidence**.
...
the edge ${e_4}$ is a loop — it connects the node ${d}$ to itself. A
...
the set 𝑀 is the set of all men in the United States (about
${162~400~000}$), and the set 𝑊 is the set of all women in the United
...
Since we're adding all of the degrees of all the nodes in 𝑀, the numerator
...
That's nice expression — we know the sizes of those sets:
...
partners than women. No surveys were needed — just the population numbers
...
𝑆, 𝑇, 𝑈, 𝑉, and 𝑊. If we use all five available time slots, the answer's
straightforward:
...
Can we do better? Sure, the course 𝑒 can use the time slot 𝑉. The only
other node assigned 𝑉 is the node 𝑐, and the nodes 𝑒 and 𝑐 are not
adjacent:
...
Now the students in course 𝑒 don't have to take their exam so late. Can we
...
— a solution to a coloring problem — is correct. Finding that coloring,
...
node for last, the algorithm will have to use more colors — it can't "go
back and redo".
