...
<head>
	...
	<description>Notes on number theory.</description>
</head>
...
7. [Sieve of Eratosthenes](#sieve-of-eratosthenes)
8. [Mersenne Primes](#mersenne-primes)
9. [Greatest Common Divisor](#greatest-common-divisor)
10. [Euclid's Algorithm](#euclids-algorithm)
11. [Application: Encryption](#application-encryption)
   ...
   3. [Congruency](#congruency)
   4. [Multiplicative Inverse](#multiplicative-inverse)
...
> **definition**. An integer is a number with no fractional component.
...
> **definition**. The set of integers is the set: $$ \{ \ldots, -2, -1, 0,
> 1, 2, \ldots \} $$ denoted ${\ZZ.}$
...
> _The Order Axiom_. Where ${~~a,b \in \ZZ,~~}$ if ${0 < a,~~}$ _and_ >
> ${0 < b}$ then:
>
> $$
> \begin{aligned}
...
> _Axiom_. If ${~~a\in \ZZ,~~}$ then
> ${~~\nexists x \in \ZZ : a < x < a+1}$.
...
> _Notation_. Suppose ${a, b \in \ZZ,}$ and ${a \neq 0.}$ If
> ${b/a \in \ZZ,}$ then we say that &#8220;${a}$ divides ${b,}$&#8221;
> denoted:
>
> $$
...
$$
 m \space\vert\space a \iff \exists k \in \ZZ : (a = k \cdot m)
$$
...
> **lemma**. Let ${a, b, c \in \ZZ.}$ Then:
>
> 1. ${a \space\vert\space b \land a \space\vert\space c \implies a \space\vert\space (b + c)}$
> 2. ${\forall c \in \ZZ : [(a \space\vert\space b) \implies (a \space\vert\space b \cdot c)]}$
...
> **lemma**. If ${a}$ divides ${b,}$ and ${a}$ divides ${c,}$ then ${a}$
> divides ${b + c.}$ For example, 2 divides 10 and 20. Following lemma 1, 2
> divides 30.

> **lemma**. If ${a}$ divides ${b,}$ then ${a}$ divides ${b}$ times ${c,}$
> for any integer ${c.}$ For example, 3 divides 30. It follows that 3
> divides 2(30), or 60.

> **lemma**. The third lemma: If ${a}$ divides ${b}$ and ${b}$ divides
> ${c,}$ then ${a}$ divides ${c.}$[^lemma_three_note]
...
> _Theorem._ Suppose ${a \in \ZZ}$ and ${a, b \in \ZZ^{+}.}$ Then:
>
> $$
>   \exists q, r \in \ZZ : (a = dq + r) \land (0 \leq r < d)
> $$
...
> _Theorem._ Suppose ${a, b \in \ZZ}$ and ${m \in \ZZ^{+}.}$ Then:
>
> $$
...
The Sieve of Eratosthenes provides a way to find primes. The idea behind
the sieve: Say we want to find the primes up to some natural number ${n.}$
The first prime we encounter is 2, so anytime we see a multiple of 2, we
ignore it and move on. The next prime we encounter is 3, so anytime we see
a multiple of 2, or a multiple of 3, we ignore it and move on. The next
prime we encounter is 5, so the anytime we see multiples of 2, 3, or 5, we
ignore it and move on. We continue this process until we've hit the prime
closest to, but less than, ${\sqrt{n}.}$

> _Euclid's Theorem._ There are an infinite number of primes.

## Mersenne Primes

A _Mersenne Prime_ is a positive integer ${M}$ that:

1. is of the form ${2^n - 1,}$ where ${n \in \PP,}$ and
2. is a prime (${M \in \PP}$)

Note that it's a common misconception that ${2^n - 1}$ yields a prime (try
${2^{11} - 1}$; hint: it's not prime). It's an open problem whether there
are infinitely many Mersenne ("muhr-sen") primes.
...
> _Theorem._ Suppose that ${a,b \in \ZZ,}$ and ${b \in \ZZ^{+}.}$ Then
> there exists a unique quotient ${q,}$ and a remainder ${r}$ of ${a}$
> divided by ${b}$ such that ${b = qa + r}$ and ${0 \leq r < b.}$
...
> **lemma**. The greatest common denominator of ${a}$ and ${b}$ is the
...
${a, b, q, r \in \ZZ^{+}.}$ Thus, what we want to show is that the largest
...
> For all ${a, b, q, r, k \in \ZZ}$ and ${b \in \ZZ^{+},}$ if
> ${a = bq + r}$ and ${k \space\vert\space a}$ and
> ${k \space\vert\space b,}$ then ${k \text{ div } r.}$
...
${a = kn}$ for some ${n \in \ZZ.}$ And since ${k \space\vert\space b,}$ it
follows that ${b = km}$ for some ${m \in \ZZ.}$ We use two different
...
${r = kt}$ for some ${t \in \ZZ.}$ We'll keep this form, ${r = kt,}$ in
mind as we continue.
...
comes the coup de grâce: Let ${t = n - mq.}$ Given that
${n, m, q \in \ZZ,}$ it follows that ${t}$ must be an integer. It follows
that ${r = kt,}$ where ${t}$ is some integer. Therefore,
${k \space\vert\space r.}$
...
<Sequence data={["v", "i", "c", "t", "o", "r", "y"]} scale={40} />
...
> **definition**. ${a}$ and ${b}$ are relatively prime iff
> ${\text{gcd}(a, b) = 1.}$
...
> **lemma**. ${\text{gcd}(a,b) = 1}$ iff there exists two integers, ${s}$
> and ${t,}$ such that ${sa + tb = 1.}$

Connecting this lemma with the previous definition by logical bicondition,
we have:

> **lemma**. ${a}$ and ${b}$ are relatively prime iff there exists two
...
${b.}$ This observation is closely related to the notion of _congruency._

### Congruency

Congruency is defined as follows:

> **definition**. ${x}$ is congruent to ${y \bmod n}$ if, and only if,
> ${n \space\vert\space (x-y).}$ If ${x}$ is conguent to ${y \bmod~n,}$ we
> write:
>
> $$
>  x \equiv y~(\bmod~n)
...
For example, the number 31 is congruent to ${16~(\bmod~5).}$ We know this
is true because:
...
And we know that 5 divides 15. Now that we have an understanding of
congruency, we need another concept, the _multiplicative inverse_.

### Multiplicative Inverse

The multiplicative inverse is defined as follows:

> **definition**. The multiplicative inverse of ${x \bmod n}$ is a number
> ${x^{-1}}$ in the interval ${[0, 1, \ldots, n-1]}$ such that
> ${x \times x^{-1} \equiv 1 \bmod n.}$

For example, we know that ${2 \times 3 = 6.}$ We can also say that 6 is
congruent to 1 module 5:

$$
 2 \times 3 = 1~(\bmod~5)
$$

Why? Because ${6 - 1 = 5,}$ and ${5}$ is divisible by ${5.}$ Importantly,
the expression ${1~(\bmod~5)}$ is a term in itself. As such, we rewrite the
expression as:
...
that is, ${2}$ is congruent to the multiplicative inverse of ${3}$ modulo
${5.}$ Alternatively, we can also say that ${3}$ is congruent to the
multiplicative inverse of ${2}$ modulo ${5}$:
...
$$
 5 \cdot 5 \equiv 1~(\bmod~6)
$$

${5 \cdot 5 = 25,}$ ${25 - 1 = 24,}$ and ${6}$ divides ${24.}$ This example
...
I.e., ${5}$ is congruent to its multiplicative inverse modulo ${6.}$ With
these basics, let's get back to our encoding scheme. Recall that we left
off on the question of how we might decode:

$$
 \text{rem}(mk, p)
$$

where ${m}$ is the message, ${k}$ is a secret prime number, and ${p}$ is a
public prime number.