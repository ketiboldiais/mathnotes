...
negative integers, it's false for the set of all integers. I.e., ${\ZZ}$ is
not well ordered. And since it's false for ${\ZZ,}$ it's false for the set
of all rational numbers, ${\mathbb{Q}.}$ And since it's false for
${\mathbb{Q},}$ it's false for the set of all real numbers, ${\RR.}$ These
all imply that there's something very special about the nonnegative
integers, a.k.a. the natural numbers ${\NN.}$[^wop]
...
> _Postulate_. Let ${m, n \in \ZZ^{+}.}$ Then ${\dfrac{m}{n}}$ can be
...
**Proof (By Contradiction)**. Let ${m, n \in \ZZ^{+}.}$ Suppose
...
Since ${A}$ is nonempty and ${A \subseteq \NN,}$ then by the well-ordering
...
> ${\forall n \in \NN : (P(n) \implies P(n + 1))}$ is true, then
> ${\forall n \in \NN : P(n)}$ is true.
...
$$
	\forall n \in \NN : \left(\sum_{i = 1}^{n} = \dfrac{n(n + 1)}{2}\right)
$$
...
$$
	\forall n \in \NN: (3 \space\vert\space n^3 - n)
$$
...
$$
	(\forall n \in \NN \mid n \geq 0) : P(n) \implies P(n + 1)
$$
...
> ${\forall n \in \NN,}$ there exists a way to tile a ${2^n \times 2^n}$
