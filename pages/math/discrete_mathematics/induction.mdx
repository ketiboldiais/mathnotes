...
__direct proof__. We now introduce another type of proof, the **indirect
...
The __Well-ordering Principle__ states:

> __definition__. Every nonempty set of nonnegative integers has a smallest
> element.
...
negative integers, it's false for the set of all integers. I.e., ${\uint}$ is
not well ordered. And since it's false for ${\uint,}$ it's false for the set
of all rational numbers, ${\mathbb{Q}.}$ And since it's false for
${\mathbb{Q},}$ it's false for the set of all real numbers, ${\reals.}$ These
all imply that there's something very special about the nonnegative
integers, a.k.a. the natural numbers ${\nat.}$[^wop]
...
> _Postulate_. Let ${m, n \in \uint^{+}.}$ Then ${\dfrac{m}{n}}$ can be
...
__Proof (By Contradiction)__. Let ${m, n \in \uint^{+}.}$ Suppose
...
Since ${A}$ is nonempty and ${A \subseteq \nat,}$ then by the well-ordering
...
The notation above is called a __summation__. There are a number of reasons
...
__base case__. Here, we must establish that ${P(0)}$ is true. We saw this
...
__inductive case__. In this case, we must show: For ${n \geq 0,}$
${P(n) \nc P(n + 1)}$ is true.
...
${P(n) \nc P(n + 1).}$ Remember, in mathematics, we write a proof one
...
${x \nc y.}$ For example, remember how we wrote a
...
true when ${P(n)}$ is true, the implication ${P(n) \nc P(n + 1)}$
cannot be false. Therefore, the implication ${P(n) \nc P(n + 1)}$ must
be true. Ergo,
${\forall n \in \nat : \left(\sum_{i = 1}^{n} = \dfrac{n(n + 1)}{2}\right)}$
is true. ${\blacksquare}$
...
inductive step: For ${n \geq 0,}$ ${P(n) \nc P(n + 1)}$ is true. More
formally:
...
> ${\forall n \in \nat,}$ there exists a way to tile a ${2^n \times 2^n}$
...
true to verify ${P(n) \nc P(n + 1).}$ Now we must prove that
${P(n + 1)}$ is true.
