...
The process of __curve sketching__ is to illustrate the graph of some
function ${f(x)}$ by using ${f'(x)}$ and ${f''(x),}$ whether ${x}$ is
positive or negative. To curve sketch, we state some key propositions up
front:

> __Lemma.__ Let ${f}$ be a differentiable function. Then the following
...
${f}$ is __concave up__. And when ${f'' < 0,}$ or when ${f'}$ is
decreasing, we say that ${f}$ is __concave down__. In the sketches above,
${f}$ is concave down. Thus, we have the following propositions:

> __lemma__. Let ${f}$ be a differentiable function. Then the following
> propositions are true:
>
...
> __notation__. Given a function ${f,}$ if ${f^{\uparrow}}$ then ${f}$ is
>
...
Notice the yellow dots above. These points are called __turning points__ or
__inflection points__. They're simply points where the derivative (i.e.,
the _slope_) changes its sign. Formally:

> __definition__. If ${f'(x_0) = 0,}$ then ${x_0}$ is a _critical point_,
...
1. Proposition 1: ${x \to (-2)^{+} \nc f(x) \to \infty^{-}}$
2. Proposition 2: ${x \to (-2)^{-} \nc f(x) \to \infty^{+}}$
