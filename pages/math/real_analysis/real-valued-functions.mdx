...
|            | ${f: x \in \X \mapsto y \in \Y}$ | ~notation~         | ~leeway~                                                            |
| ---------- | -------------------------------- | ------------------ | ------------------------------------------------------------------- |
| injective  | ${y}$ mapped to _at most once_   | ${f: \X \inj \Y}$  | some ${y}$ are not mapped to                                        |
| surjective | ${y}$ mapped to _at least once_  | ${f: \X \surj \Y}$ | some ${y}$ are mapped to more than once (but not by the same ${x}$) |
| bijective  | ${y}$ mapped to _exactly once_   | ${f: \X \bij \Y}$  | none; unique ${x}$ to unqiue ${y}$                                  |
