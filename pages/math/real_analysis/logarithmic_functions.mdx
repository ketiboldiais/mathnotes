...
import { Plot as Plot2 } from "../../../components/hago/";
...
1. ${b^x = ?}$ maps to the question: What do I get if I multiply the number
   ${b}$ by itself ${x}$ times?

2. ${\log_{b}(y) = ?}$ maps to: How many times can I divide ${y}$ by ${b}$
   to get the number ${y?}$

If these two questions feel related, they are. Notice the symmetry in
notation:

|       Exponent       |           Logarithm           |
| :------------------: | :---------------------------: |
| $$ \large y = b^x $$ | $$ \large x = \log\_{b}(y) $$ |
...
> __definition__. Where ${x > 0,}$ ${b > 0,}$ and ${b \neq 1,}$
>
> $$
...
> __definition__ . Given ${f(x) = \log_{b}(x),}$
>
> $$
> 	\text{dom}(f) = (0, \infty) \\
...
And to illustrate, compare the graph of ${f(x) = 2^x}$ with
${g(x) = \log_{2}(x)}$:

<Plot
	functions={[
		{ f: (x) => 2 ** x, color: "var(--red)" },
		{ f: (x) => Math.log2(x), color: "var(--green)" },
	]}
	geo={[
		{ type: "label", id: "f(x) = 2^x", fill: "var(--red)", xy: [-6, 3] },
		{
			type: "label",
			id: "g(x) = \\log_2(x)",
			fill: "var(--green)",
...
If ${b \in \reals^{-},}$ then we arrive at some difficulty. Remember,
...
Because the natural domain of the functions we are dealing with is ${\reals,}$
the logarithmic function is _undefined_ where ${b \in \reals^{-}.}$ Now
...
> __definition__. In mathematics, the _common logarithm_ is defined as:
>
> $$
...
> __definition__. In computer science, the _binary logarithm_ is defined as:
>
> $$
...
> __definition__. The _natural logarithm_ is a logarithm of base ${e.}$
>
> $$
...
-1. Accordingly, ${ln(-1) \nc \text{undefined}.}$ In other words,
...
${g(x) = b^x,}$ where ${b > 0, b \neq 1,}$
${\text{dom}(g) = (-\infty, \infty) = \reals.}$ And
...
$$
\begin{aligned}
		\text{dom}(t) &= \{ x \in \reals \mid 2x - 3 > 0 \} \\
		&= \{ x \in \reals \mid 2x > 3 \}  \\
		&= \{ x \in \reals \mid x > 3/2 \} \\
		&= \{ x \in \reals \mid x > 1.5 \} \\
		&\equiv (1.5, \infty)
\end{aligned}
$$
...
<Plot
	functions={[
		{ f: (x) => Math.log(x) / Math.log(1.25), color: "var(--lavender)" },
		{ f: (x) => Math.log(x) / Math.log(2), color: "var(--red)" },
		{ f: (x) => Math.log(x) / Math.log(4), color: "var(--blue)" },
		{ f: (x) => Math.log(x) / Math.log(10), color: "var(--green)" },
	]}
	geo={[
		{
			type: "label",
