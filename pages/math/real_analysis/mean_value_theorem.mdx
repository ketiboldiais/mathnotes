# Mean Value Theorems
There are several mean value theorems. The most important among them is Lagrange's Mean Value Theorem, often abbreviated as MVT.

> __~lagrange's mean value theorem~.__ Let ${f:D \subseteq \reals \to \reals.}$ If ${f}$ is [continuous](#continuous-functions) on the closed interval ${\cic{a}{b}}$ and [differentiable](#differentiable-functions) on the open interval ${\oio{a}{b},}$ then there exists a ${c \in \oio{a}{b}}$ such that
> 
> $$
> 	f'(c)=\dfrac{f(b)-f(a)}{b-a}.
> $$

If we satisfy the requirements for Lagrange's MVT, then we know that there's _at least one_ value ${c \in \oio{a}{b}}$ such that the tangent line to the curve ${y=f(x)}$ at ${x=c}$ has a gradient, or slope, equal to the gradient of the secant line passing through the points ${(a,f(a))}$ and ${(b,f(b)).}$

<Fig
	link={"https://res.cloudinary.com/sublimis/image/upload/v1666334890/math/mvt1_khbzp9.svg"}
	imwidth={"516"}
	imheight={"311"}
	caption={"mvt1"}
	width={"70"}
...
On its own, Lagrange's MVT isn't all that interesting. Its implications, however, have wide-ranging consequences.

> __~first corollary of mvt~.__ Let ${f}$ be a function [continuous](#continuous-functions) on the closed interval ${\cic{a}{b}}$ and differentiable on the open interval ${\oio{a}{b}.}$ If ${f'(x)=0}$ for all ${x \in \oio{a}{b},}$ then ${f(x)}$ is a _constant function_ on ${\cic{a}{b}.}$

> __~second corollary of mvt~.__ Let ${f}$ and ${g}$ be two functions continuous on the closed interval ${\cic{a}{b}}$ and differentiable on the open interval ${\oio{a}{b}.}$ If ${f'(x)=g'(x),}$ for all ${x \in \oio{a}{b},}$ then there exists a constant ${C}$ such that ${f(x)=g(x)+C}$ for all ${x \in \cic{a}{b}.}$
...
> __Mean Value Theorem__. Let ${f(x)}$ be a function where the following
> conditions are true:
>
...
> __lemma__. Where the mean value theorem is true, the following
> propositions hold:
>
...
> __problem__. Is ${e^x}$ greater than ${1+x+\dfrac{x^2}{2}?}$
