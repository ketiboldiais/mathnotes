...
> Definition. For any ${x \in \reals,}$ an exponential function is a function
> of the form ${f(x) = ab^x.}$ Where:
>
> - ${a \in \reals^*}$ (${\reals^* = \reals \setminus \{ 0 \}}$)
> - ${b \in \reals^{+}}$

The domain of ${f}$ is the set of all real numbers ${\reals.}$ If ${a > 0,}$
then ${\text{ran}(f) \in \reals^{+}.}$ If ${a < 0,}$ then
${\text{ran}(f) \in \reals^{-}.}$ Further:
...
> Definition. For every ${x \in \reals,}$ ${\{a, b \in \reals\ \mid b \neq 1\},}$
...
> $$
>
> where ${x \in \reals,}$ ${a, b \in \reals,}$ and ${b < 1.}$
...
> represented by __Euler's constant__ ${e,}$ whose approximate value is
> ${2.718282.}$
...
> Definition. The __continuous growth function__ ${G}$ is a function of the
> form ${G(t) = ve^{gt}}$ where ${t \in \reals,}$ ${v, g \in \reals^{+},}$ >
...
> Definition. The __continuous decay function__ is a function of the form:
> ${D(t) = ve^{dt}}$ where ${t \in \reals,}$ ${v, r \in \reals^{+},}$ ${r < 0,}$
...
3. If the problem refers to _continuous growth_, then ${r > 0}$ (i.e.,
   ${r \in \reals^{+};}$ ${r}$ is a positive real number).
4. If the problem refers to _continuous decay_, then ${r < 0}$ (i.e.,
   ${r \in \reals^{-};}$ ${r}$ is a negative real number).
...
If ${C \in \reals^{+},}$ then ${G(f)}$ shifts _up_ the ${y \text{-axis}}$ by
${C}$ units. If ${C \in \reals^{-},}$ then ${G(f)}$ shifts _down_ the
...
the following: (i) ${\text{dom}(f) = \{ x \in \reals \};}$ i.e., the domain
...
${\text{ran} = \{ f(x) \in \reals \mid f(x) > C \}.}$
...
${\text{ran} = \{ f(x) \in \reals \mid f(x) > -C \},}$ while the domain remains
unchanged, ${\{ x \in \reals \}.}$
...
${(- \infty , \infty),}$ ${\reals,}$ remains unchanged. (2) The horizontal
...
4. If ${C \in \reals^{+},}$ shift ${G(f)}$ _left_ ${C}$ units.
5. If ${C \in \reals^{-},}$ shift ${G(f)}$ _right_ ${C}$ units.
6. If ${d \in \reals^{+},}$ shift ${G(f)}$ _up_ ${d}$ units.
7. If ${d \in \reals^{-},}$ shift ${G(f)}$ _down_ ${d}$ units.
8. State the domain, ${\reals,}$ and the range, ${d, \infty,}$ and the
...
The last possible transformation is __reflection__. When we multiply the
