<Metadata
	...
	description={`Notes on Gauss's Method`}
	keywords={`Gaussian elimination, linear algebra`}
/>

# Gaussian Elimination

This chapter provides an overview of _Gaussian elimination_.
...
Like all mathematical fields, linear algebra has its own lingo. Let's
familiarize ourselves with some of it. An expression of the form:
...
_column_ index. Much like how rectangles are described with ${\tt{length} \times
\tt{width},}$ linear systems have the property ${m \times n.}$ We call this the
...
\tt{row 1}.}$" The pivoting operation is expressed as:
